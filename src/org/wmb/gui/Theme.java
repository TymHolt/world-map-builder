package org.wmb.gui;

import org.wmb.rendering.Color;

import java.awt.*;

public final class Theme {

    public static final Color BACKGROUND =
        new Color(40.0f / 255.0f, 44.0f / 255.0f, 52.0f / 255.0f, 1.0f);
    public static final Color BACKGROUND_LIGHT =
        new Color(60.0f / 255.0f, 63.0f / 255.0f, 70.0f / 255.0f, 1.0f);
    public static final Color FOREGROUND =
        new Color(230.0f / 255.0f, 230.0f / 255.0f, 230.0f / 255.0f, 1.0f);
    public static final Color BORDER =
        new Color(60.0f / 255.0f, 60.0f / 255.0f, 65.0f / 255.0f, 1.0f);
    public static final Color HIGHLIGHT =
        new Color(100.0f / 255.0f, 150.0f / 255.0f, 250.0f / 255.0f, 1.0f);
    public static final Color ELEMENT_ICON =
        new Color(238.0f / 255.0f, 75.0f / 255.0f, 43.0f / 255.0f, 1.0f);
    public static final Font FONT_PLAIN = new Font("Liberation Mono", Font.PLAIN, 35);
    public static final Font FONT_BOLD = new Font("Liberation Mono", Font.BOLD, 35);
}
